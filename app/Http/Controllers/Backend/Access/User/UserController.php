...
class UserController extends Controller
{
    ...
    {                               
        $this->users->create(['data' => $request->except('assignees_roles'), 
            'roles' => $request->only('assignees_roles')]);
        return redirect()->route('admin.access.user.index')->withFlashSuccess('El usuario ha sido creado correctamente');
    }
    /**
     * @param User              $user
     * @param ManageUserRequest $request
    ...
    public function destroy(User $user, ManageUserRequest $request)
    {
        ...
        return redirect()->route('admin.access.user.deleted')->withFlashSuccess(trans('El usuario ha sido eliminado correctamente'));
    }
}
